...
struct CSCSDDMMImpl {
  ...
  void operator()(CSCSDDMMArgs& args) const
  {
    ...
    CSCSDDMMImplBody<KIND, INDEX_CODE, VAL_CODE>()(
      A_vals, B_pos, B_crd, B_vals, C_vals, D_vals, args.D_vals.shape<2>(), args.B_crd.shape<1>());
  }
};

template <VariantKind KIND>
static void csc_sddmm_template(TaskContext& context)
{
  auto& inputs = context.inputs();
  if (inputs[0].transformed()) { inputs[0].remove_transform(); }
  CSCSDDMMArgs args{context.outputs()[0], inputs[0], inputs[1], inputs[2], inputs[3], inputs[4]};
  index_type_value_type_dispatch(args.B_crd.code(), args.A_vals.code(), CSCSDDMMImpl<KIND>{}, args);
}
