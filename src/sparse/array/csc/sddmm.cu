...
struct CSCSDDMMImplBody<VariantKind::GPU, INDEX_CODE, VAL_CODE> {
                  ...
                  const Rect<1>& vals_rect)
  {
    ...
    sddmm_csc_kernel<<<blocks, THREADS_PER_BLOCK, 0, stream>>>(nnzs,
                                                               vals_rect.lo[0],
                                                               buf.ptr(0),
...
/*static*/ void CSCSDDMM::gpu_variant(TaskContext& context)
{
  csc_sddmm_template<VariantKind::GPU>(context);
}
