...
void ElemwiseMultCSRCSRNNZ::cpu_variant(TaskContext& context)
{
  csr_elem_mult_nnz_template<VariantKind::CPU>(context);
}

void ElemwiseMultCSRCSR::cpu_variant(TaskContext& context)
{
  csr_elem_mult_template<VariantKind::CPU>(context);
}
...
{
  ElemwiseMultCSRCSRNNZ::register_variants();
  ElemwiseMultCSRCSR::register_variants();
}
}  // namespace
