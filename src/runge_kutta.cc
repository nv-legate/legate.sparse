...
void RKCalcDy::cpu_variant(legate::TaskContext& ctx)
{
  ...
  if (dy.transformed()) { dy.remove_transform(); }
...
#pragma unroll
    for (int64_t j = 0; j < s; j++) { acc += K_acc[{j, i}] * a_acc[j]; }
    dy_acc[i] = acc * h;
  }
}
...
{
  sparse::RKCalcDy::register_variants();
}
}  // namespace
