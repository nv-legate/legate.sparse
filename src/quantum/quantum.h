...
struct IntSet {
 public:
  ...
  {
    int count = 0;
    for (int i = 0; i < (N * sizeof(T) * 8); i++) { count += int(this->is_index_set(i)); }
    return count;
  }

  friend bool operator<(const IntSet<N, T>& a, const IntSet<N, T>& b) { return a.bits < b.bits; }
...
#define INTSET_DISPATCH(x, n)                                      \
  switch ((n + (sizeof(bit_ty) * 8) - 1) / (sizeof(bit_ty) * 8)) { \
    case 1: x<1, bit_ty>(ctx); break;                              \
    case 2: x<2, bit_ty>(ctx); break;                              \
    case 3: x<3, bit_ty>(ctx); break;                              \
    case 4: x<4, bit_ty>(ctx); break;                              \
    case 5: x<5, bit_ty>(ctx); break;                              \
    case 6: x<6, bit_ty>(ctx); break;                              \
    case 7: x<7, bit_ty>(ctx); break;                              \
    case 8: x<8, bit_ty>(ctx); break;                              \
    case 9: x<9, bit_ty>(ctx); break;                              \
    case 10: x<10, bit_ty>(ctx); break;                            \
    case 11: x<11, bit_ty>(ctx); break;                            \
    case 12: x<12, bit_ty>(ctx); break;                            \
