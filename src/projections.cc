...
Legion::LogicalRegion Promote1Dto2DFunctor::project(Legion::LogicalPartition upper_bound,
  ...
  auto color_space = this->runtime->get_index_partition_color_space(upper_bound.get_index_partition());
...
Functor1DToRowsImplicit2D::Functor1DToRowsImplicit2D(Legion::Runtime *rt, int32_t gx, int32_t gy, bool rows) : LegateSparseProjectionFunctor(rt), gx(gx), gy(gy), rows(rows) {}
...
void register_legate_sparse_1d_to_2d_functor(legion_projection_id_t proj_id, int32_t gx, int32_t gy, bool rows) {
  ...
  rt->register_projection_functor(proj_id, new sparse::Functor1DToRowsImplicit2D(rt, gx, gy, rows), true /* silence_warnings */);
}
}
