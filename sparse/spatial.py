...
def cdist(XA, XB, metric="euclidean", *, out=None, **kwargs):
    ...
    # piece within the rows.
    task.add_input(
        XA_store.partition(
            Tiling(Shape((x_tiling, XA.shape[1])), Shape((grid[0], 1)))
        ),
        proj=lambda p: (p[0], 0),
    )
    # We do a similar thing for XB, but this time we'll take the projection of
    # the columns of the processor grid.
    task.add_input(
        XB_store.partition(
            Tiling(Shape((y_tiling, XB_store.shape[1])), Shape((grid[1], 1)))
        ),
        proj=lambda p: (p[1], 0),
    )
    task.execute()
